...
(function() {
  ...
  function renderOnboarding(user) {
    ...
    html += '<p>Get the full trading experience on your mobile device. Available on <span data-app-label>the App Store or Google Play</span>.</p>';
    html += '<div class="download-buttons" data-app-store-section>';
    html += '<a href="https://play.google.com/store/apps/details?id=com.mambamanagement.app" data-app-link="android" target="_blank" rel="noopener noreferrer" class="store-btn google-play">';
    ...
    html += '</a>';
    html += '<a href="https://apps.apple.com/app/id6776478463" data-app-link="ios" target="_blank" rel="noopener noreferrer" class="store-btn app-store">';
    ...
    // These steps are rendered after config.js has already run, so re-apply
    // the platform-aware store links to the markup we just injected.
    if (window.MAMBA_CONFIG && window.MAMBA_CONFIG.applyAppLinks) {
      window.MAMBA_CONFIG.applyAppLinks();
    }
